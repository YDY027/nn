...
from carla_autonomous.srv import StartEpisode, Stop, Reset
# 导入项目模块
...
class CarlaAutonomousROS:
        ...
        with self.lock:
            try:
                ...
                if self.env:
                    ...
                    # 返回当前状态
                    return ResetResponse(current_state)
                else:
                    rospy.logerr("环境未初始化")
    ...
    def run_episode(self):
        ...
        try:
            ...
            if done:
                ...
                status_msg = f"Episode {self.current_episode} 中断，步数: {self.step_count}"
    ...
    def run(self):
        ...
        while not rospy.is_shutdown():
            try:
                if self.running and self.env and self.env.vehicle:
                    # 发布数据
                    self.publish_all_data()
